const PRODUCTS = {
      ...
      'Softdrinks': [
        { id: 'cola', name: 'Cola/Fanta/Sprite (0,33l)', price: 2.50, desc: '0,33l' },
        { id: 'cola125', name: 'Cola/Fanta/Sprite (1,25l)', price: 7.00, desc: '1,25l' },
    ...
    function getImgUrl(name) {
      if (imageMap[name]) return 'menus/popular menus/' + imageMap[name];
    ...
    function renderMenu(searchQuery = '') {
      ...
      Object.entries(PRODUCTS).forEach(([category, products]) => {
        ...
        filteredProducts.forEach(product => {
          ...
          let imgUrl = getImgUrl(product.name);
    ...
    function openCustomModalOrAdd(id) {
      ...
      if (isCustomizable) {
        ...
        const imgUrl = getImgUrl(productName);
        ...
        const imgUrl2 = getImgUrl(productName);
    ...
    function checkout() {
      ...
      Object.entries(cart).forEach(([id, qty]) => {
        ...
        Object.values(PRODUCTS).forEach(category => {
          ...
          if (product) {
            const customizations = id.includes('|') ? ` (${id.split('|')[1]})` : '';
            items.push(`${qty}x ${product.name}${customizations}`);
          }
        });
      });
